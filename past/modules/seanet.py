...
class SEANetEncoder(nn.Module):
    ...
    Args:
        ...
        transformer_params (dict): Parameters for the transformer.
    """
    ...
    def __init__(
        ...
        inflating_factor: int = 2,
        transformer_params: tp.Optional[tp.Dict[str, tp.Any]] = None,
    ):
        super().__init__()
        ...
        for i, ratio in enumerate(self.ratios):
            ...
            for j in range(n_residual_layers):
                model += [
                    SEANetResnetBlock(
                        mult * n_filters,
                        kernel_sizes=[residual_kernel_sizes[i], 1],
            ...
            model += [
                ...
                StreamableConv1d(
                    mult * n_filters,
                    mult * n_filters * inflating_factor,
                    kernel_size=ratio * inflating_factor,
                    stride=ratio,
                    norm=block_norm,
    ...
    def forward(self, x, return_cnn_features: bool = False):
        x_cnn = self.model(x)
        if self.encoder_transformer is not None:
            x = self.encoder_transformer(x_cnn)
        else:
            x = x_cnn
...
class SEANetDecoder(nn.Module):
    ...
    def forward(self, z):
        if self.decoder_transformer is not None:
            z, _ = self.decoder_transformer(z)
        y = self.model(z)
        return y
