...
from past.environment import AudioCraftEnvironment
from past.cluster import get_slurm_parameters
...
def get_solver(cfg):
    from past.solvers.builders import get_solver
...
@hydra_main(config_path='../config', config_name='config_sslcodec', version_base='1.1')
def main(cfg):
    ...
    if cfg.execute_only:
        assert cfg.execute_inplace or cfg.continue_from is not None, (
            "Please explicitly specify the checkpoint to continue from with continue_from=<sig_or_path> " + "when running with execute_only or set execute_inplace to True."
        )
